...
export class ExperimentInstanceEditorComponent implements OnInit {
  ...
  public selectedED: ExperimentDescription | undefined;
  /** Local data to host experiment descriptions to choose from (to associate with the experiment instance) */
  public experimentDescriptions: ExperimentDescription[] = [];

  constructor(private edm: LabchainDatabase) { }

  /** Form to hold the information associated with this instance */
  public experimentInstanceForm: FormGroup = new FormGroup({
   ...
   */
  public suggestID(): number {
    let idRange = 1;
    let freeIDFound = false;
    let suggestedID = Math.floor(Math.random() * idRange);
    while (!freeIDFound) {
      suggestedID = Math.floor(Math.random() * idRange);
