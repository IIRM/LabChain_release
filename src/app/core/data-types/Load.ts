...
export class Load extends DispatchableAsset {
  ...
  constructor(
    ...
    readonly loadCurve?: number[]
  ) {
    super(model);
    this.recentFlexibilityUse = false;
    this.currentLoad = 0;
    if(loadCurve === undefined) {
      this.loadProfile = Load.getRandomizedSLP("transition");
      LoadOperationLogicService.randomizeSLP(this.loadProfile);
    } else {
      this.loadProfile = this.loadCurve;
    }
    this.powerSeries = this.loadProfile;
    // for testing purposes only:
    // this.powerSeries = new Array(this.loadProfile.length).fill(1);
    this.initiateShiftingPotential();
  }
  ...
  // TODO test me! currently more load can be shifted as intended! Problem with redefinition of the time steps?
  /**
   * Initiates shifting potential for each time step that can be shift to another time step
   */
  private initiateShiftingPotential() {
    this.shiftingPotential = Array<Array<number>>();

    for (let i = 0; i <= this.loadProfile.length; i++) {
      const row: number[] = new Array<number>(this.loadProfile.length + 1);// todo length +1?
      row.fill(undefined);
    ...
    this.shiftingPotential.forEach((entry, index) => {
      ...
      if (maxIndex > this.loadProfile.length) {
        maxIndex = this.loadProfile.length;
      }
  ...
  public getLoadAtTimeStep(time: number): number {
    return this.loadProfile[time];
  }

  /**
   * returns a standard load profile depending on the season, i.e. "winter", "summer", "transition"
   *
   * @param season specifies time of the year
   */
  static getRandomizedSLP(season: String): number[] {
    switch(season) {
      case "winter":
        return [338.5, 233.2, 172.5, 158.2, 154.0, 156.5, 187.5, 360.3, 517.6, 534.0, 492.8, 464.9, 473.3, 522.8,
      ...
      case "summer":
        return [432.3, 294.4, 219.3, 199.1, 187.5, 195.2, 237.0, 391.6, 518.9, 566.5, 576.3, 557.0, 567.8, 631.2,
      ...
      case "transition":
        return [375.9, 266.4, 196.7, 177.1, 172.4, 176.2, 210.5, 371.7, 518.9, 549.9, 543.8, 528.2, 533.0, 598.4,
          ...
          510.5, 375.9]; // transition season, i.e. spring or autumn */

      default:
        console.error("Illegal season in LabchainDatabase.getRandomizedSLP: " + season);
        return [];
    }
  }
}
